...
import AppBar from 'fitness-client/node_modules/material-ui/AppBar/AppBar.js';
import MuiThemeProvider from 'fitness-client/node-modules/material-ui/MuiThemeProvider';
import TextField from 'fitness-client/node-modules/material-ui/TextField';
import RaisedButton from 'fitness-client/node-modules/material-ui/RaisedButton';
import {Tabs, Tab} from 'fitness-client/node-modules/material-ui/material-ui/Tabs';
import FloatingActionButton from 'fitness-client/node-modules/material-ui/FloatingActionButton';
import CheckIcon from 'fitness-client/node-modules/material-ui/material-ui/svg-icons/action/check-circle';
import ListIcon from 'fitness-client/node-modules/material-ui/material-ui/svg-icons/action/list';
import TodoIcon from 'fitness-client/node-modules/material-ui/material-ui/svg-icons/action/today';
import EditIcon from 'fitness-client/node-modules/material-ui/material-ui/svg-icons/action/delete';
import CloseIcon from 'fitness-client/node-modules/material-ui/material-ui/svg-icons/content/delete-sweep';
import ColumnList from '../ColumnList';
import ConfirmDialog from '../ConfirmDialog';
import If from '../If';
import '/fitness-client/src/style/ExerciseLog.css';
...
class ExerciseLog extends Component {
	...
	/**
	 * @description Add task to the Exercises-To-Do list.
	 */
	addTask = () => {
	...
	/**
	 * @description Update task toggling between Exercises-To-Do/Done status.
	...
	handleUpdateTask = (task) => {
		this.setState(previousState => {
			...
			task.status = (task.status === 'Exercises-To-Do') ? 'Completed' : 'Exercises-To-Do';
	...
	render() {
		...
		const columns = [
			{ title: 'Exercises-To-Do', items: items.filter( item => item.status === 'Exercises-To-Do'), icon: <TodoIcon />},
			{ title: 'Completed ', items: items.filter( item => item.status === 'Completed'), icon: <CheckIcon />},
			...
			<MuiThemeProvider>
				<div className="App">
					...
					<ConfirmDialog
						...
						message={'Are you sure you want to remove all exercises from the App?'}
						onCancel={this.handleDialogClose}
						onConfirm={this.clearTasks}
						open={this.state.dialogOpen}
					/>
					<AppBar
						title={<span style={{color: 'white'}}>Exercise Tracker</span>}
					...
					<div className="App-container">
						<div style={{position: 'fixed', width: '100%', paddingTop: 64, zIndex: 8888, backgroundColor: 'white'}}>
							<TextField
								hintText="Type exercise"
								floatingLabelText="Add Exercise"
								ref={(taskInput) => {
...
ExerciseLog.propTypes = propTypes;
ExerciseLog.defaultProps = defaultProps;
